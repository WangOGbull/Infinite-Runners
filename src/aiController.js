...
class AIController {
  ...
  getInputAngle(dragon, allDragons) {
    ...
    const now = Date.now();
    const stillCommitted = dragon.aiHuntTarget && dragon.aiHuntTarget.alive &&
      dragon.aiHuntUntil && now < dragon.aiHuntUntil &&
    ...
    if (bestTarget) {
      ...
      dragon.aiHuntTarget = null;
      dragon.aiHuntUntil = 0;
    }
    ...
    if (dragon.aiTargetAngle !== null && dragon.aiTargetAngle !== undefined) {
      ...
      while (diff < -Math.PI) diff += Math.PI * 2;
      targetAngle = dragon.aiTargetAngle + diff * turnResponsiveness;
    }
